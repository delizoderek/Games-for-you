...
function carouselImg(){
    fetch(`https://api.rawg.io/api/games?page_size=20&key=${rawgApi}`)
    .then(function(data){
        return data.json();
    })
    .then(function(data){
        ...
        document.getElementById("carName1").textContent = data.results[index].name
        car3.setAttribute("src", `${data.results[index2].background_image}`)
        document.getElementById("carName3").textContent = data.results[index2].name
    })
    fetch(`https://api.boardgameatlas.com/api/search?random=true&client_id=${bgAtlasApi}`)
    .then(function(data){
        return data.json()
    })
    .then(function(data){
